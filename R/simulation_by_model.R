...
label_to_membership <- function(labels, label.names = NULL){
    if (is.null(label.names)){
        ...
        checkmat::assert_true(all(labels %in% label.names))
    }
...
# set.seed(42)
# K = 6
# data.out = generate_data(n=2000, p=500, ntask=5, K=K, cl.sep=2, sig=1, batch.effect.sig=1, alpha=0.5)
# X = do.call(rbind, data.out$X.list)
...
#
#
# umap.model.x = uwot::umap(X, ret_model = TRUE, min_dist = 0.5, n_neighbors = 50, metric = "cosine", pca=30)
# df.plot =  data.frame(UMAP_1 = umap.model.x$embedding[,1],
#                       UMAP_2 = umap.model.x$embedding[,2],
#                       labels = labels,
#                       batch = batchs)
# ggplot(df.plot, aes(x=UMAP_1, y=UMAP_2, col=labels)) + theme_light()+
#             geom_point(size=1, stroke=0, type=16, alpha=1) +
#          guides(col=guide_legend(ncol=1, title='Cell type', override.aes = list(size=5)))
#
#
# source('R/logger.R')
# source('R/data_integration.R')
# int.out = data_integrate(data.out$X.list, r=6, verbose=T,
#                            max.niter=100,  tol=1e-5,
#                            init = NULL,  nrep=1, seed=0)
# # int.out = data_integrate(data.out$X.list, r=5, gamma1=0, gamma2=0, gamma3=0, gamma4=1e6,
# #                          max.niter=5,  tol=1e-5,
# #                          init = int.out, verbose=T, nrep=1, seed=0)
#
# Hest = do.call(rbind, int.out$H.list)
# umap.model.h = uwot::umap(Hest, ret_model = TRUE, min_dist = 0.1, n_neighbors = 50, metric = "cosine", pca=NULL)
# df.plot =  data.frame(UMAP_1 = umap.model.h$embedding[,1],
#                       UMAP_2 = umap.model.h$embedding[,2],
#                       labels = labels,
#                       batch = batchs)
# ggplot(df.plot, aes(x=UMAP_1, y=UMAP_2, col=labels)) + theme_light()+
#     geom_point(size=1, stroke=0, type=16, alpha=1) +
