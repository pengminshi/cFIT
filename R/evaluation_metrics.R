...
calculate_alignment <-function(X.list, k=NULL, balanced=F, min.sample.per.dataset = 100,
    ...
    if (verbose) logmsg('Calculate alignment of ',m,' datasets ...')
    ...
    if (balanced){
        ...
        if (verbose) logmsg('Subsample tp ',min.sample.per.dataset,' points per dataset ...')
    ...
    if (verbose) logmsg('Find k nearest neighbor with k = ', k,'...')
    ...
    if (verbose) logmsg('Calculate alignment score')
    ...
    if(!is.null(labels)){
        if (verbose) logmsg('Aggregate alignment score per cluster ...')
    ...
    if (verbose) logmsg('Aggregate alignment score per dataset ...')
...
calculate_alignment_per_type <-function(X, dataset=NULL, labels=NULL, k=NULL,
    ...
    if (verbose) logmsg('Aggregate alignment per cluster per dataset')
    alignment.per.dataset.cluster = aggregate(alignment.per.sample, by=list(dataset=dataset, labels=labels), FUN=mean, na.rm=T)
...
#'
#' @importFrom FNN get.knn
get_cluster_match <- function(exprs.source, exprs.target,
...
#'
#' @importFrom FNN get.knn
asign_labels <- function(exprs.source, exprs.target,
                         labels.source, k=NULL, max.k=50, verbose=F){
