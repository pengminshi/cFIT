...
CFITIntegrate_sketched <- function(X.list, r = 15, max.niter = 100, tol = 1e-04,
                                   gamma = 1e+06, nrep = 1, init = NULL,
                                   subsample.prop = NULL, weight.list = NULL,
    ...
    for (rep in 1:nrep) {
        ...
        if (all(c("W", "lambda.list", "b.list", "H.list") %in% names(init))) {
            ...
            params.list = initialize_params(X.list = X.list.sub, r = r, gamma = gamma, W = init$W, verbose = verbose, n.cores=n.cores)
            # params.list = initialize_params_random(X.list = X.list, r = r) # random initialization
        }
        ...
        for (iter in 1:max.niter) {
            ...
            # estimate the membership matrix first
            # random permute update order for the other three sets of parameters
            ...
            for (params.to.update in params.to.update.list) {
                ...
                params.list = solve_subproblem_penalized(  params.to.update = params.to.update, X.list = X.list.sub,
        ...
        params.list = solve_subproblem(params.to.update = 'H', X.list = X.list,
                                       W = params.list$W, H.list = params.list$H.list,
                                       b.list = params.list$b.list, lambda.list = params.list$lambda.list,
    ...
    return(list(H.list = params.list.best$H.list, W = params.list.best$W, b.list = params.list.best$b.list,
                ...
                niter = niter.best, params = list(gamma = gamma, max.niter = max.niter, tol = tol, nrep = nrep,
                                                  subsample.prop = subsample.prop, weight.list = weight.list, n.cores = n.cores)))
}
...
solve_subproblem_penalized <- function(params.to.update = c("W", "lambda", "b", "H"), X.list, W, H.list,
                             b.list, lambda.list, gamma, iter, params.list.last, verbose = T, n.cores) {
    params.to.update = match.arg(params.to.update)
    m = length(X.list)

    if (params.to.update == "W") {
        W = solve_W_penalized(X.list = X.list, H.list = H.list, lambda.list = lambda.list, b.list = b.list,
                              W.old=params.list.last$W, iter=iter, n.cores = n.cores)
    } else if (params.to.update == "lambda") {
        lambda.list = solve_lambda_list_penalized(X.list = X.list, W = W, H.list = H.list, b.list = b.list, gamma = gamma,
                                                  lambda.list.old=params.list.last$lambda.list, iter=iter, n.cores = n.cores)
    } else if (params.to.update == "b") {
        b.list = lapply(1:m, function(j) solve_b_penalized(X.list[[j]], W = W, H = H.list[[j]], lambd = lambda.list[[j]],
                                                           b.old = params.list.last$b.list[[j]], iter=iter,n.cores = n.cores))
    } else {
...
#' @import checkmate lsei
solve_lambda_list_penalized <- function(X.list, W, H.list, b.list, gamma, lambda.list.old, iter, n.cores) {
    ...
    if (m > 1) {
        lambda.out = parallel::mclapply(1:nrow(W), function(l) {
            ...
            Amat <- diag(sapply(Ajl.list, function(Ajl) sum(Ajl^2))) +
                gamma * matrix(nvec, ncol = 1) %*% matrix(nvec, nrow = 1)/ntotal # m*m matrix
