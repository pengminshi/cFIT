...
int.out = CFITIntegrate(X.list=exprs.list, r=15, max.niter = 100, seed=42, verbose=F)
saveRDS(int.out, file='results/panc5_integration_output.rds')
```
...
```{r, eval=T, fig.align='center',fig.show="hold", fig.cap='Integrated pancreas islet cells data sests from five technologies.'}
...
# run time 20 min
int.ref.out = CFITIntegrate(X.list=exprs.list[c(1,2,3,5)], r=15, max.niter = 100, seed=42, verbose=F)
...
tf.out = CFITTransfer(Xtarget=exprs.list[[4]], Wref=int.ref.out$W, max.niter = 100, seed=42, verbose=F)
```
...
```{r, eval=T}
int.out = CFITIntegrate(X.list=data.out$X.list, r=6, max.niter = 100, seed=42, verbose=F) 
...
```{r, echo=F, fig.align='center',  fig.cap='Examine the run time (left) and objective function with sketched cFIT with varying subsampling fractions.'}
knitr::include_graphics('fig/simu_sketched.png')
```
